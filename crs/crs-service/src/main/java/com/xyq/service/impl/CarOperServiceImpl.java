...
import com.xyq.mapper.CarOperMapper;
...
public class CarOperServiceImpl implements CarOperService {
	 ...
	 */
	@Override
	public SlfPage selectAllSLFByOUOService(SlfLog slfLog, Integer num, Integer pageSize) {
		
		//将查询页码转换为起始查询行号
		int pageStart = num*pageSize-pageSize;
		//获取总的数据量
		int count = carOperMapper.selectCountSLFByThree(slfLog);
		//获取总的分页数量
		int pages = (int) Math.ceil(count*1.0/pageSize);
		//获取分页数据
		List<SlfLog> cs = carOperMapper.selectAllSLFByOUO(slfLog, pageStart, pageSize);
