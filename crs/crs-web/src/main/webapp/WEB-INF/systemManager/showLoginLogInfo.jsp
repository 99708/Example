...
<div class="container">
	<div id="inner-hd">
		<div class="crumbs">
			...
			<a href="javascript:;" class="crumbs-label">登录信息</a>
		</div>
	</div>
		...
		<div class="year-summary">
			<div class="kv-group-outer">
				<div class="kv-group clearfix">
					...
					<div class="kv-item">
						<div class="item-lt">Ip地址：</div>
						<div class="item-rt">
							<input type="text" id="ip" name="ip" value="" style="width: 210px; height:33px;border:1px solid #c2c2c2;padding:0 5px"/>
						</div>
					</div>
					<div class="kv-item">
						<div class="item-lt">登录时间：</div>
						<div class="item-rt">
							<input type="text" id="time" name="time" value="" onClick="WdatePicker()" style="width: 210px; height:33px;border:1px solid #c2c2c2;padding:0 5px"/>
						</div>
					</div>
			...
			<div class="table-zone">
				<table>
					...
					<tr>
						...
						<td class="comment">用户名</td>
						<td class="comment">Ip地址</td>
						<td class="comment">登录时间</td>
					</tr>
					</thead>
					<tbody id="tb">
					</tbody>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $("#export").click(function(){
            post('<%=request.getContextPath() %>/exportLoginExcel',{"login":rData});
        });
    ...
    function getData(pNum, pSize){
        ...
        tb.empty();
        $.ajax({
            url: "<%=request.getContextPath() %>/selLoginInfo",
            data: "uname=" + $("#uname").val() + "&ip=" + $("#ip").val()
            + "&time=" + $("#time").val() + "&pageNum=" + pNum + "&pageSize=" + pSize,
            dataType: "json",
            type: "post",
            success: function (rsData) {
                ...
                rData = JSON.stringify(rsData.loginList);

                for (var i = 0; i < rsData.loginList.length; i++) {
                    var login = rsData.loginList[i];
                    tb.append(
                        "<tr>"+
                        "<td>"+(i+1)+"</td>"+
                        "<td>"+login.user.uname+"</td>"+
                        "<td>"+login.ip+"</td>"+
                        "<td>"+login.time.substr(0,10)+"</td>"+
                        "</tr>");
                }
